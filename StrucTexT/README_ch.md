...
### 字符级别实体分类
   * 数据集
      ...
      * 字符级别实体分类包含的类型如下：
      ```
        学科、测试时间、姓名、学校、考试号、座位号、班级、学号、年级、分数。
      ```

   * 效果：不同设置下的StrucTexT模型在EPHOIE数据集的效果如下：

    | 模型                               | <strong>字段级别 F1 score</strong> |
    | :-------------------------------- | :-------------------------------: |
...
### 字段级别实体分类
   ...
   * 效果: 不同设置下的StrucTexT模型在SROIE、FUNSD以及XFUND数据集的效果如下：
    ...
    | 模型                        | <strong>SROIE</strong>          | <strong>FUNSD</strong>          | <strong>XFUND-ZH</strong>          |
    | :-----------------------------| :-----------------------------: | :-----------------------------: | :-----------------------------:  |
...
### 字段级别实体连接
   ...
   * 效果: 不同设置下的StrucTexT模型在FUNSD以及XFUND-ZH数据集的效果如下（在所有可能的语义实体连接对中预测的**F1 score**）：
    ...
    | 模型                               |      <strong>FUNSD</strong>       |      <strong>XFUND-ZH</strong>    |
    | :-------------------------------- | :-------------------------------: | :-------------------------------: |
...
| [StrucTexT Base for FUNSD linking](http://aisee.bj.bcebos.com/VIMER/StrucTexT/StrucTexT_base_funsd_linking.pdparams.tar.gz?authorization=bce-auth-v1%2Fdb0b41e2ab894ecfb1126a768c603d79%2F2021-12-02T08%3A50%3A16Z%2F-1%2Fhost%2F50aee9546d618b296ec1dfc54084e497ee4836f7034dbe6e2131c26a4003f870)       | 181 |
