...
Feature = namedtuple("Feature", [
    ...
    'sentence_mask', 'cls_label', 'link_label', 'label_mask'])
...
def convert_examples_to_features(config, examples, tokenizer, transform=None):
    ...
    cls_label = np.zeros((batch_size, max_seqlen), dtype='int32')
    link_label = np.zeros((batch_size, max_seqlen, max_seqlen), dtype='int32')
    ...
    for idx in range(batch_size):
        ...
        # assign label
        cls_label[idx, :line_len] = cls[:line_len]
        linkings = _flatten_2d_list(linkings[:line_len])
        linkings = list(filter(lambda x: x[0] in ids and x[1] in ids, linkings))
        ...
        label_mask[idx, :line_len - 1] = True
        for linking in linkings:
            start_id = ids[linking[0]]
            end_id = ids[linking[1]]
            link_label[idx][start_id][end_id] = 1

    cls_label = cls_label[:, :max_vislen]
    link_label = link_label[:, :max_vislen, :max_vislen]
