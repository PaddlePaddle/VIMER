...
class EASTProcessTrain(object):
    ...
    def check_and_validate_polys(self, polys, tags, classes, img_height, img_width):
        """
        ...
        validated_tags = []
        validated_classes = []
        for poly, tag, cls in zip(polys, tags, classes):
    ...
    def generate_quad(self, im_size, polys, tags, classes):
        """
        Generate quadrangle.
        """
        h, w = im_size
        ...
        for poly_idx, poly_tag in enumerate(zip(polys, tags, classes)):
            poly = poly_tag[0]
            tag = poly_tag[1]
            clas = poly_tag[2]
            ...
            cv2.fillPoly(score_map, shrinked_poly, int(clas) + 1)
    ...
    def crop_area(self, im, polys, tags, classes, crop_background=False, max_tries=50):
        """
    ...
    def crop_background_infor(self, im, text_polys, text_tags, text_classes):
        """ crop_background_infor """
        if self.train_mode:
            im, text_polys, text_tags, text_classes = self.crop_area(
                    im, text_polys, text_tags, text_classes, crop_background=True)
        ...
        im_shape = np.array([src_h, src_w, ratio, ratio])
        ...
        return im, score_map, geo_map, training_mask, \
               im_shape, text_polys, text_tags, text_classes

    def crop_foreground_infor(self, im, text_polys, text_tags, text_classes):
        """ crop_foreground_infor """
        if self.train_mode:
            im, text_polys, text_tags, text_classes = self.crop_area(
                    im, text_polys, text_tags, text_classes, crop_background=False)
        ...
        score_map, geo_map, training_mask = self.generate_quad(
            (new_h, new_w), text_polys, text_tags, text_classes)
        return im, score_map, geo_map, training_mask, \
               im_shape, text_polys, text_tags, text_classes
    ...
    def __call__(self, data):
        ...
        text_classes = data.get('labels',
                np.zeros_like(text_tags, 'int32').tolist())
        if im is None:
            return None
        if text_polys.shape[0] == 0:
            return None
        ...
        h, w, _ = im.shape
        text_polys, text_tags, text_classes = self.check_and_validate_polys(text_polys,
                                                              text_tags, text_classes, h, w)
        if text_polys.shape[0] == 0:
            return None
        ...
        if self.train_mode:
            ...
            text_polys *= rd_scale
        if self.train_mode and np.random.rand() < self.background_ratio:
            outs = self.crop_background_infor(im, text_polys, text_tags, text_classes)
        else:
            outs = self.crop_foreground_infor(im, text_polys, text_tags, text_classes)

        if outs is None:
            return None
        im, score_map, geo_map, training_mask, \
            im_shape, text_polys, text_tags, text_classes = outs
