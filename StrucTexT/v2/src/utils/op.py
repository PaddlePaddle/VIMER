...
def batch_norm(ch, act=None, norm_type='bn', name=None):
    """batch_norm"""
    bn_name = 'bn' if name is not None else name + '.bn'
    if paddle.distributed.get_world_size() > 1 \
            and norm_type == 'sync_bn':
...
def roi_align(input,
    ...
    Args:
        x (Tensor): Input feature, 4D-Tensor with the shape of [N,C,H,W], 
            where N is the batch size, C is the input channel, H is Height, W is weight. 
            The data type is float32 or float64.
        boxes (Tensor): ROIs (Regions of Interest) to pool over.It should be
    ...
    Examples:
        .. code-block:: python
            ...
            boxes = paddle.static.data(
                name='rois', shape=[None, 4], dtype='float32')
            boxes_num = paddle.static.data(name='rois_num', shape=[None], dtype='int32')
    ...
    if in_dygraph_mode():
        ...
        check_variable_and_dtype(x, 'input', ['float32', 'float64'],
                                 'roi_align')
        check_variable_and_dtype(boxes, 'rois', ['float32', 'float64'],
                                 'roi_align')
