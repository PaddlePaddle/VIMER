...
class Model(Encoder):
    def __init__(self, config, name=''):
        ...
        self.input_size = self.config['in_channels']
        self.elem_num = 2
    ...
    def forward(self, *args, **kwargs):
        ...
        batch_size = enc_final.shape[0]

        last_shape = int(np.prod(enc_final.shape[2:]))  # gry added
        fea = P.reshape(enc_final, [enc_final.shape[0], enc_final.shape[1], last_shape])
        fea = fea.transpose([0, 2, 1])  # (NTC)(batch, width, channels)

        out_enc = self.encoder(fea)
        batch_size = out_enc.shape[0]
        row_index = P.reshape(P.to_tensor(list(range(500))), (1, -1))
        row_index = P.tile(row_index, (batch_size, 1))
        col_index = P.reshape(P.to_tensor(list(range(250))), (1, -1))
        col_index = P.tile(col_index, (batch_size, 1))

        row_probs, row_m_probs, row_features = self.row_decoder(enc_final, out_enc, row_index)
        col_probs, col_m_probs, col_features = self.col_decoder(enc_final, out_enc, col_index)
        link_probs = self.link_decoder(enc_final, row_features, col_features)
...
class PositionalEncoding(nn.Layer):
    def __init__(self, d_model, dropout=0., max_len=50000):
        ...
        pe = P.zeros((max_len, d_model))
        position = P.arange(0, max_len).unsqueeze(1).astype('float32')
        div_term = P.exp(P.arange(0, d_model, 2).astype('float32') * -math.log(10000.0) / d_model)
...
class PositionalEncoding2D(nn.Layer):
    ...
    def __init__(self, d_model, dropout=0., max_len=5000):
        ...
        pe = P.zeros([max_len, d_model])
        position = P.arange(0, max_len, dtype=P.float32).unsqueeze(1)
        div_term = P.exp(
            P.arange(0, d_model, 2).astype('float32') *
...
def self_attention(query, key, value, mask=None, dropout=None, pred_layer=False):
    ...
    """
    d_k = value.shape[-1]
    score = P.matmul(query, key.transpose([0, 1, 3, 2]) / math.sqrt(d_k))
    if mask is not None:
        # pass
        # score = score.masked_fill(mask == 0, -1e9) # b, h, L, L
        # score = score.masked_fill(mask == 0, -6.55e4) # for fp16
        # score = score[mask == 0] = -6.55e4
        tmp = P.ones(shape=score.shape, dtype='float32') * -1e9
        mask = P.tile(mask, repeat_times=[1, score.shape[1], 1, 1])
        score = P.where(mask == 0, tmp, score)
    if not pred_layer:
    ...
    return P.matmul(p_attn, value), p_attn
...
class RowtDecoder(nn.Layer):
    ...
    def decode(self, image_fea, feature, cell_index, src_mask, tgt_mask):
        ...
        image_fea = P.transpose(image_fea, [0, 2, 3, 1])
        row_fea = P.mean(image_fea, axis = 2)
...
class ColtDecoder(nn.Layer):
    ...
    def decode(self, image_fea, feature, cell_index, src_mask, tgt_mask):
        ...
        image_fea = F.upsample(image_fea, size=[500,250], mode="bilinear", align_mode=1)
        image_fea = P.transpose(image_fea, [0, 2, 3, 1])
        col_fea = P.mean(image_fea, axis = 1)
        x = self.embedding(cell_index)
        for i, layer in enumerate(self.layers):
            x, atten = layer(x, feature, src_mask, tgt_mask)
...
class LinktDecoder(nn.Layer):
    ...
    def decode(self, img_features, row_features, col_features, row_mask, col_mask):
        ...
        row_features = P.transpose(P.tile(P.unsqueeze(row_features, 2), [1, 1, 250, 1]), [0, 3, 1, 2])
        col_features = P.transpose(P.tile(P.unsqueeze(col_features, 1), [1, 500, 1, 1]), [0, 3, 1, 2])
        ...
        return self.corner_fc(P.concat([corner_features, img_features], axis = 1))
