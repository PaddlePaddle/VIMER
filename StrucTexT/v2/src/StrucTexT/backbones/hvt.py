...
from .base.droppath import DropPath
...
class HVT(nn.Layer):
    ...
    def __init__(self,
                 ...
                 droppath=0.):
        super().__init__()
        self.out_channels = [embed_dim] * (depth // pool_block_width + 1)
        self.pool_block_width = pool_block_width
    ...
    def convert(self, x):
        """ convert """
        x = paddle.transpose(x, [0, 2, 1]) #(B, C, P)
        p = x.shape
        wh = int(math.sqrt(p[-1]))
        assert wh ** 2 == p[-1]
        x = x.reshape([p[0], -1, wh, wh])
        return x
    ...
    def forward(self, x, **kwargs):
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x)
            if (i + 1) % (2 * self.pool_block_width) == 0:
                out.append(self.convert(x))

        x = self.norm(x)
        return {'out': x,
                'additional_info': {'all_feats': out}
               }
