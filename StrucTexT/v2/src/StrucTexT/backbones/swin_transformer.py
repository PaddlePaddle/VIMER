...
MODEL_URLS = {
    ...
    "../output/SwinTransformer_base_patch4_window12_384_pretrained.pdparams",
...
class SwinTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.mlp_ratio = mlp_ratio
        self.out_channels = [embed_dim * 2 ** i for i in range(self.num_layers)]
    ...
    def forward(self, x):
        ...
        out = []
        before_downsampling = self.before_downsampling
        out_indices = self.out_indices
        if out_indices[0] == 0 and before_downsampling is False:
            wh = int(math.sqrt(x.shape[1]))
            out.append(x.transpose([0, 2, 1]).reshape([0, x.shape[-1], wh, wh]))
            out_indices = [e - 1 for e in out_indices[1:]]
        else:
            out_indices[-1] = out_indices[-1] - 1

        x = self.pos_drop(x)
        for idx, layer in enumerate(self.layers):
            x_no_down, x = layer(x)
            m = x_no_down if before_downsampling else x
            if idx in out_indices:
                wh = int(math.sqrt(m.shape[1]))
                out.append(m.transpose([0, 2, 1]).reshape([0, m.shape[-1], wh, wh]))
        return {'out': x,
                'additional_info': {
                    'all_feats': out}
               }
