...
class EASTPostProcess(object):
    ...
    def __call__(self, outs_dict, shape_list):
        ...
        num_class = score_list.shape[1]
        score_list = paddle.nn.functional.softmax(score_list, axis=1)

        if isinstance(score_list, paddle.Tensor):
            score_list = score_list.numpy()
            geo_list = geo_list.numpy()
        if isinstance(shape_list, paddle.Tensor):
        ...
        for ino in range(img_num):
            ...
            for idx in range(1, num_class):
                ...
                if len(boxes) > 0:
                    ...
                    boxes[:, :, 0] /= ratio_w
                    boxes[:, :, 1] /= ratio_h
                    for i_box, (box, ss) in enumerate(zip(boxes, box_s)):
