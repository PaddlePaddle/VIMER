...
def convert_examples_to_features(config, examples, tokenizer, transform=None):
    ...
    for idx in range(batch_size):
        ...
        # expand cls
        extend_cls = [0] + _flatten_2d_list(cls)
        extend_cls = [[c] * len(id) for c, id in zip(extend_cls, extend_tokens)] # expand to subword
        extend_cls = _flatten_2d_list(extend_cls) # concat all subword_bboexes
        ...
        cls_len = len(extend_cls)
        assert(token_len == cls_len)
        ...
        # assign label
        label[idx, :cls_len] = extend_cls
        label_mask[idx, 1:cls_len] = 1
