...
class ReidEvaluatorSingleTask(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        prediction = {
            'feats': outputs.cpu(), #paddle.to_tensor(outputs, place=self._cpu_device, dtype=paddle.float32),
            'pids': inputs['targets'].cpu(), #paddle.to_tensor(inputs['targets'], place=self._cpu_device, dtype=inputs['targets']),
            'camids': inputs['camids'].cpu(), #paddle.to_tensor(inputs['camids'], place=self._cpu_device, dtype=inputs['camids']),
        }
