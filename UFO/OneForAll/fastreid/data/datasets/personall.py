...
VERSION_DICT = {
    'PersonAll': {
        TRAIN_DIR_KEY: 'mask_train_v3',
        TEST_DIR_KEY: 'mask_test_v3',
    }
}


@DATASET_REGISTRY.register()
class PersonAll(ImageDataset):
    """MSMT17.
    Reference:
    ...
    Dataset statistics:
        ...
        - cameras: 15.
    """
    dataset_dir = 'PersonAll'
    dataset_url = None
    dataset_name = 'PersonAll'

    def __init__(self, root='datasets', **kwargs):
        has_main_dir = False
        if osp.exists(osp.join(root, self.dataset_dir)):
            train_dir = VERSION_DICT[self.dataset_dir][TRAIN_DIR_KEY]
            test_dir = VERSION_DICT[self.dataset_dir][TEST_DIR_KEY]
            has_main_dir = True
        ...
        super(PersonAll, self).__init__(train, query, gallery, **kwargs)
