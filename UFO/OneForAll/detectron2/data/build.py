...
def print_instances_class_histogram(dataset_dicts, class_names):
    ...
    for entry in dataset_dicts:
        ...
        if len(classes):
            assert classes.min() >= 0, "Got an invalid category_id={classes.min()}"
            assert (
                classes.max() < num_classes
            ), "Got an invalid category_id={classes.max()} for a dataset of {num_classes} classes"
...
def build_detection_train_loader(
    ...
    *,
    ...
    if isinstance(dataset, torchdata.IterableDataset):
        ...
        if sampler is None:
            sampler = TrainingSampler(len(dataset))
        assert isinstance(sampler, torchdata.Sampler), "Expect a Sampler but got {type(sampler)}"
