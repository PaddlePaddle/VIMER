...
def _patch_import():
    ...
    def find_relative_file(original_file, relative_import_path, level):
        ...
        if not PathManager.isfile(cur_file):
            raise ImportError(
                "Cannot import name {relative_import_path} from "
                "{original_file}: {cur_file} has to exist."
            )
        return cur_file
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    ...
                    "{filename} is human-readable but cannot be loaded."
                )
                save_pkl = True
        except Exception:
        ...
        def safe_update(cfg, key, value):
            ...
            for idx in range(1, len(parts)):
                ...
                if not OmegaConf.is_config(v):
                    raise KeyError(
                        "Trying to update key {key}, but {prefix} "
                        "is not a config, but has type {type(v)}."
                    )
        ...
        def _to_str(obj, prefix=None, inside_call=False):
            ...
            if isinstance(obj, abc.Mapping) and "_target_" in obj:
                ...
                return (
                        "{"
                        + ",".join(
                    "{repr(k)}: {_to_str(v, inside_call=inside_call)}"
                    for k, v in sorted(obj.items())
                )
                        + "}"
                )
