...
def assert_instances_allclose(input, other, *, rtol=1e-5, msg="", size_as_tensor=False):
    ...
    size_error_msg = msg + "image_size is {input.image_size} vs. {other.image_size}!"
    if size_as_tensor:
        assert torch.equal(
    ...
    assert fields == fields_other, msg + "Fields are {fields} vs {fields_other}!"
    ...
    for f in fields:
        ...
        if isinstance(val1, (Boxes, ROIMasks)):
            ...
            if val1.dtype.is_floating_point:
                ...
                assert torch.equal(val1, val2), msg + "Field {f} is different!"
        else:
