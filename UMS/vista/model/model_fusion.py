...
class ViSTA(nn.Layer):
    ...
    def __init__(self, args):
        ...
        self.fusion_projection = nn.Linear(self.image_embed_dim, self.projection_dim, bias_attr=False)
    ...
    def encode_image(
        ...
        if scene_text_features is not None and scene_text_mask is not None:
            return self.image_cls.backbone(image, scene_text_features.astype('float32'), \
    ...
    def forward(
        ...
        fusion_embeds = fusion_embeds / paddle.norm(fusion_embeds, axis=-1, keepdim=True)
