...
__all__=['fusion_small_patch16_224']
...
class VisionStFusionTransformer(nn.Layer):
        ...
        **kwargs
        ...
        self.scene_text_fusion_blocks = nn.LayerList([
            Block(
                ...
                drop_path=dpr[i],
                norm_layer=norm_layer,
                epsilon=epsilon) for i in range(fusion_depth)
        ])
        ...
        self.scene_text_feature_embeddings = nn.Linear(scene_text_feature_size, embed_dim)
    ...
    def forward_features(self, x, scene_text_features=None, scene_text_mask=None):
        """forward_features
        """
        ...
        for blk in self.blocks[:-1 * fusion_depth]:
            img_features = blk(img_features, mask=None)
        scene_text_features = self.scene_text_feature_embeddings(scene_text_features)
        fusion_token = self.fusion_token.expand((paddle.shape(x)[0], -1, -1))
        fusion_token_token_type_embed = self.token_type_embeddings(
                paddle.full(shape=[paddle.shape(fusion_token)[0], \
                    paddle.shape(fusion_token)[1]], fill_value=2, dtype='int64'))
        fusion_token = fusion_token + fusion_token_token_type_embed + self.fusion_pos_embed
        image_fusion_attention_mask = paddle.ones(shape=[paddle.shape(x)[0], paddle.shape(x)[1] + 1], dtype='int64')
        fusion_attention_mask = paddle.ones(shape=[paddle.shape(x)[0], 1])
        scene_text_fusion_attention_mask = paddle.concat((fusion_attention_mask, scene_text_mask), axis=1)
        for i in range(self.fusion_depth):
            image_fusion_x = paddle.concat((img_features, fusion_token), axis=1)
            image_fusion_x = self.blocks[-1 * fusion_depth + i](image_fusion_x, mask = image_fusion_attention_mask)
            scene_text_fusion_x = paddle.concat((fusion_token, scene_text_features), axis=1)
            scene_text_fusion_x = self.scene_text_fusion_blocks[i](scene_text_fusion_x,\
                    mask = scene_text_fusion_attention_mask)
    ...
    def forward(self, x, scene_text_features=None, scene_text_mask=None):
        """forward
        """
        x = self.forward_features(x, scene_text_features=scene_text_features, scene_text_mask=scene_text_mask)
        return x


def fusion_small_patch16_224(fusion_depth=4, **kwargs):
    """fusion_small_patch16_224
    """
