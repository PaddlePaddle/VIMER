...
class LocalSceneTextDataset(Dataset):
    ...
    def __init__(
            ...
            for filename in pos_filenames:
                ...
                with open(filename) as f:
                    for line in f.readlines():
                        ...
                        self.imageID_to_pos[image_id].append([conts[0], conts[1], conts[4], conts[5], conts[8]])
            print("{}, {} have {} ocr text".format(txt_file, pos_path, len(self.imageID_to_pos.keys())))    
            ...
            for line in f:
                ...
                if is_train:
                    ...
                    if return_ocr_feature == 3 and image_id not in self.imageID_to_pos:
                        print("warning: {}, {} not find ocr text".format(txt_file, image_id), self.imageID_to_pos)
        ...
        self.scene_text_tokenizer = BertTokenizer.from_pretrained(scene_text_token_conf_path)
    ...
    def pro_ocr_txt(self, image_id, w, h):
        ...
        for data in pos_datas:
            ...
            for i in range(len(scene_text_split)):
                pos.append([int(x_min) * 1.0 / w, int(y_min) * 1.0 / h, 
                    int(x_max) * 1.0 / w, int(y_max) * 1.0 / h])
        
        scene_texts = ' '.join(scene_texts)
        #print(self.scene_text_tokenizer.tokenize(scene_texts))
        scene_text_ids, scene_text_mask, scene_text_segment_ids = process_sent(scene_texts, \
            self.scene_text_tokenizer, self.max_scene_text_length)
        ...
        return [scene_text_ids, scene_text_mask, scene_text_segment_ids, scene_text_status, pos]
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            if self.return_ocr_feature == 3:
                ...
                scene_text_ids, scene_text_mask, scene_text_segment_ids, scene_text_status, \
                    pos = self.pro_ocr_txt(image_id, w, h)
                res = [image, input_ids, input_mask, segment_ids, \
                    scene_text_ids, scene_text_mask, scene_text_segment_ids, scene_text_status]
                ...
                if ocr_feature_idx >= 0 and self.ocr_features is not None:
                    ocr_feature = self.ocr_features[ocr_feature_idx][:self.ocr_feature_max_len][:]
                else:
                    ocr_feature = np.zeros((self.ocr_feature_max_len, self.ocr_feature_dim))
                ocr_mask = []
                ...
                return [image, input_ids, input_mask, segment_ids, \
                        ocr_feature, ocr_mask, segment_ids, segment_ids]
            else:
                if not self.is_train:
