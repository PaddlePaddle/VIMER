...
import models.modeling_attentive_probing as modeling_attn
...
def get_args():
    ...
    parser.add_argument('--linear_depth', default=1, type=int,
                        help='num of blocks of linear module')
    parser.add_argument('--linear_pos_embed', action='store_true', default=False)
    parser.add_argument('--linear_type', default='simple', choices=['simple', 'standard'],
                        type=str, help='type of the decoder blocks')
    
    parser.add_argument('--no_parameter', action='store_true', default=False, help='if set as True, then the simple decoder block will have no parameters.')
...
def main(args, ds_init):
    ...
    if True:  # args.distributed:
        ...
        if args.dist_eval:
            ...
            sampler_val = padddle.io.SequenceSampler(dataset_val)
    else:
    ...
    model = modeling_attn.__dict__[args.model](
        pretrained=False,
    ...
    if args.enable_linear_eval:
        ...
        for name, param in model.named_parameters():
            ...
            if name in ['%s.weight' % linear_keyword, '%s.bias' % linear_keyword] or head_norm in name:
                judge_requires_grad = True
            if 'linear_blocks' in name:
                judge_requires_grad = True
            if 'query_token' in name:
                judge_requires_grad = True
            if 'gamma' in name:     # gamma = 1
                judge_requires_grad = False
