...
class PatchEmbed(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        #  stride=patch_size, weight_attr=w_attr_1, bias_attr=b_attr_1)
        self.proj = nn.Conv2D(in_chans, embed_dim, kernel_size=patch_size,
          stride=patch_size, bias_attr=True)
...
class VisionTransformer(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 use_mean_pooling=True, init_scale=0.001, lin_probe=False, args=None):
        super().__init__()
        ...
        if use_abs_pos_emb:
            ...
            self.pos_embed = self.create_parameter([1, num_patches + 1, embed_dim], default_initializer=zeros_)
            self.pos_embed.set_value(self.build_2d_sincos_position_embedding(embed_dim))
    ...
    def forward_features(self, x, is_train=True):
        ...
        if self.pos_embed is not None:
            if self.use_abs_pos_emb:
                x = x + self.pos_embed.expand([batch_size, -1, -1]).astype(x.dtype).clone().detach()
            else:
                x = x + self.pos_embed.expand([batch_size, -1, -1]).astype(x.dtype).clone().detach()
                # x = x[:,1:] + self.pos_embed.expand([batch_size, -1, -1]).astype(x[:,1:].dtype).to(x.device).clone().detach()
                # x = paddle.concat([x[:,:1],x],axis=1)
