...
class VisionTransformer(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if lin_probe:
            if args.linear_type == 'simple':
                ...
                self.fc_norm = LP_BatchNorm(embed_dim)
        ...
        self.head.weight.set_value(init_scale * self.head.weight)
        self.head.bias.set_value(init_scale * self.head.bias)
    ...
    def forward_features(self, x, is_train=True):
        ...
        if self.args.linear_pos_embed:
            key_value_pos_embed = self.pos_embed.expand([batch_size, -1, -1]).astype(x.dtype).clone().detach()
        else:
            key_value_pos_embed = 0
