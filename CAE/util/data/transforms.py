...
class RandomResizedCropAndInterpolationWithTwoPic(object):
    ...
    def __call__(self, img):
        ...
        interpolation = _pil_interpolation_to_str[interpolation]
        if self.second_size is None:
            return F.resize(F.crop(img, i, j, h, w), self.size, interpolation)
        else:
            return F.resize(F.crop(img, i, j, h, w), self.size, interpolation), \
                   F.resize(F.crop(img, i, j, h, w), self.second_size, self.second_interpolation)
...
class RandomResizedCropAndInterpolation:
    ...
    def __call__(self, img):
        ...
        return F.resize(cropped_img, self.size, _pil_interpolation_to_str[interpolation])
